...
class BrosSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.pe_type == "pdpdq_ws":
            ...
            head_q_pos = head_q_pos.view(
                seq_length, batch_size, n_head, d_head)
            head_k_pos = head_k_pos.view(
                seq_length, batch_size, n_head, d_head)
            head_q_pos = head_q_pos.permute([1, 2, 0, 3])
            head_k_pos = head_k_pos.permute([1, 2, 0, 3])

            bbox_pos_scores_1 = torch.einsum(
                "bnid,bnjd->bnij", (torch.mul(query_layer,
            ...
            bbox_pos_emb = bbox_pos_emb.view(
                seq_length, seq_length, batch_size, d_head)
